function outFile = Changmin_Sheets_Merge(inDir, outFile)
%MERGE_D2D_FILES  폴더 안의 여러 엑셀 파일을 하나로 합칩니다.
%
%   merge_d2d_files()
...
%   들어가고, 2열부터 N+1열까지는 각 파일의 DrainI 가 한 파일당 한 열씩 붙습니다.
%
%   열 순서와 열 이름은 파일 이름에 들어 있는 숫자로 정합니다. 이름 어디에
%   있든 '처음 나오는 숫자'를 소자 번호로 보므로 접두어는 무엇이든 상관없고
%   (D1, Dev1, sample1_left 모두 1번), 번호 순으로 정렬한 뒤 헤더를
%   DrainI_D1, DrainI_D2 ... 형태로 적습니다. 숫자가 없는 파일이나 번호가
%   겹치는 파일이 있으면 오류로 알립니다.
%
...
%       merge_d2d_files('D2D', '다른이름.xlsx');
    ...
    % ---------------------------------------------------------------------
    % 1) 폴더 안의 엑셀 파일을 모아 파일 이름에서 뽑은 숫자 순으로 정렬합니다.
    ...
    files   = sort(files);                    % 재현 가능한 기본 순서
    baseNm  = strings(numel(files), 1);
    fileNum = zeros(numel(files), 1);
    for k = 1:numel(files)
        [~, baseNm(k)] = fileparts(files(k));
        % 이름 어디에 있든 '처음 나오는 숫자'를 소자 번호로 씁니다. 접두어가
        % D 든 Dev 든 아무것도 없든 상관없고, 'D1_left' 처럼 숫자 뒤에 글자가
        % 더 붙어 있어도 잡아냅니다.
        tok = regexp(baseNm(k), '\d+', 'match', 'once');
        if isempty(tok)
            error('merge_d2d_files:noNumberInName', ...
                  ['"%s" 파일 이름에 숫자가 없어 소자 번호를 정할 수 없습니다. ' ...
                   '파일 이름에 번호를 넣어 주세요.'], files(k));
        end
        fileNum(k) = str2double(tok);
    end

    % 번호가 열 이름까지 결정하므로 겹치면 안 됩니다. 겹치면 어느 열이 어느
    % 파일인지 구분할 수 없으므로 임의로 고르지 않고 중단합니다.
    [uniqNum, iFirst] = unique(fileNum, 'stable');
    if numel(uniqNum) < numel(fileNum)
        dupNum  = setdiff(1:numel(fileNum), iFirst);
        dupList = strjoin(compose("%s(D%d)", baseNm(dupNum), fileNum(dupNum)), ', ');
        error('merge_d2d_files:duplicateNumber', ...
              '파일 이름에서 뽑은 번호가 겹칩니다: %s', dupList);
    end

    [~, order] = sortrows([fileNum, (1:numel(files))']);   % 번호 순으로 정렬
    files   = files(order);
    baseNm  = baseNm(order);
    fileNum = fileNum(order);
    devNm   = "D" + string(fileNum);          % 열 이름에 쓸 소자 이름 (D1, D2, ...)

    nFile = numel(files);
    fprintf('%s 폴더에서 엑셀 파일 %d개를 찾았습니다: %s\n', ...
            inDir, nFile, strjoin(compose("%s->%s", baseNm, devNm), ', '));
    ...
    if numel(unique(rowCount)) > 1
        ...
        error('merge_d2d_files:cycleMismatch', ...
              'Cycle 조건이 통일되어 있지 않습니다. (파일별 행 수: %s)', detail);
    end
    nRows = rowCount(1);
    ...
    %    1열은 스윕축 이름(DrainV 또는 GateV), 2열부터는 파일 이름에서 뽑은
    %    번호를 붙여 DrainI_D1, DrainI_D2 ... 로 적습니다.
    % ---------------------------------------------------------------------
    header = [cellstr(axisNm(1)), cellstr("DrainI_" + devNm(:)')];
    if isfile(outFile)
