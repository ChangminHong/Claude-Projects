...
if (excute == 1)
    ...
    input_data     = [repmat([input_unit_set_mode; input_unit_reset_mode], PARA.NUMB_OF_SET, 1)];
...
% ----------------------------------
    ...
    reset_mode_start_point = buf(1);
    [reset_endurance.current, reset_endurance.cycle] =...
        myavg_readpulse(...
            caculated_data.currnet,...
            PARA.NUMB_OF_SET,...
            reset_mode_start_point,...
            set_duration_end + read_duration_end * 2 + reset_duration_end,...
    ...
    fig = plot(reset_endurance.cycle, reset_endurance.current, 'bo');
    title('Output Voltage')
    xlabel("number of pulse")
    ylabel("Ampere (I)")
...
% ----------------------------------
currentFolder = pwd;
    if ~isfolder('c_EnduranceVer3')
        mkdir c_EnduranceVer3
    end
    cd(currentFolder);
    cd('c_EnduranceVer3');
    ...
    writematrix((set_endurance.cycle)', strcat(save_index, '_set_endurance.cycle.txt'));
    writematrix((set_endurance.current)', strcat(save_index, '_set_endurance.current.txt'));
    writematrix((reset_endurance.cycle)', strcat(save_index, '_reset_endurance.cycle.txt'));
    writematrix((reset_endurance.current)', strcat(save_index, '_reset_endurance.current.txt'));
    saveas(fig, strcat(save_index, '_figure.fig'));
    cd(currentFolder);
end

fprintf("c_EnduranceVer3 END\n");
%   Copyright (c) 2022 by ENTIS, All rights reserved.
% 
%   File name  : c_EnduranceVer3.m
