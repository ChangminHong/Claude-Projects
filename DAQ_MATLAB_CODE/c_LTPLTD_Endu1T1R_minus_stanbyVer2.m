...
PARA.RESET_HIGH_VOLT = -1;
PARA.READ_HIGH_VOLT  = 1;
PARA.SET_DRAIN_VOLT    = -3;
PARA.RESET_DRAIN_VOLT  = -4;
PARA.READ_GATE_VOLT  = -4;
PARA.TR_PROGRAM_VOLT = 0;
PARA.TR_READ_VOLT    = 0;

PARA.SET_PULSE_DURATION   = 0.04;       % Uint : millisec
PARA.RESET_PULSE_DURATION = 0.04;
PARA.READ_PULSE_DURATION  = 0.04;

PARA.SET_AFTER_INTERVAL   = 0.04;     % Uint : millisec
PARA.RESET_AFTER_INTERVAL = 0.04;     % low
PARA.READ_AFTER_INTERVAL  = 0.04;
...
%make read mode tr voltage function
[ tr_read_unit_pulse_input, tr_read_high_start, tr_read_high_end, tr_read_duration_end ]...
    = mypulse_gen...
            (PARA.TR_READ_VOLT, PARA.READ_PULSE_DURATION,...
             PARA.READ_PULSE_DUTY_CYCLE, PARA.READ_AFTER_INTERVAL, myDAQ.Rate );
         
%make read mode gate voltage function
[ gate_read_unit_pulse_input, gate_read_high_start, gate_read_high_end, gate_read_duration_end ]...
    = mypulse_gen...
            (PARA.READ_GATE_VOLT, PARA.READ_PULSE_DURATION,...
             PARA.READ_PULSE_DUTY_CYCLE, PARA.READ_AFTER_INTERVAL, myDAQ.Rate );
...
%make set mode voltage function
[ set_drain_unit_pulse_input ]...
    = mypulse_gen...
            (PARA.SET_DRAIN_VOLT, PARA.SET_PULSE_DURATION,...
              PARA.SET_PULSE_DUTY_CYCLE, PARA.SET_AFTER_INTERVAL, myDAQ.Rate);
          
%make set mode voltage function
[ reset_drain_unit_pulse_input ]...
    = mypulse_gen...
            (PARA.RESET_DRAIN_VOLT, PARA.RESET_PULSE_DURATION,...
              PARA.RESET_PULSE_DUTY_CYCLE, PARA.RESET_AFTER_INTERVAL, myDAQ.Rate);
          
%make set mode tr voltage function
[ tr_set_unit_pulse_input, tr_set_high_start, tr_set_high_end, tr_set_duration_end ]...
    = mypulse_gen...
            (PARA.TR_PROGRAM_VOLT, PARA.SET_PULSE_DURATION,...
              PARA.SET_PULSE_DUTY_CYCLE, PARA.SET_AFTER_INTERVAL, myDAQ.Rate);
...
%make reset mode tr voltage function
[ tr_reset_unit_pulse_input, tr_reset_high_start, tr_reset_high_end, tr_reset_duration_end ]...
	= mypulse_gen...
            (PARA.TR_PROGRAM_VOLT, PARA.RESET_PULSE_DURATION,...
             PARA.RESET_PULSE_DUTY_CYCLE, PARA.RESET_AFTER_INTERVAL, myDAQ.Rate );

myavg_assert(read_high_start, read_high_end, PARA.READ_MEAN_START_PERCENT, PARA.READ_MEAN_END_PERCENT );

input_unit_set_mode         = [ [[set_unit_pulse_input, set_drain_unit_pulse_input], tr_set_unit_pulse_input];...
                                [[gate_read_unit_pulse_input, read_unit_pulse_input], tr_read_unit_pulse_input] ];

input_unit_reset_mode       = [ [[reset_unit_pulse_input, reset_drain_unit_pulse_input], tr_reset_unit_pulse_input];...
                                [[gate_read_unit_pulse_input, read_unit_pulse_input], tr_read_unit_pulse_input] ];
% ------------------------------
...
xlabel("Scan Data (#)")
ylabel("Voltage (V)")
legend({'GATE', 'DRAIN', 'TR'},'Location','best')
grid on
...
%
% -------------------------------
reconfirm = 'output 0 : GATE, output 1 : Drain, output 2 : TR\nThat Sweep is what you want? ( 1 : YES, else : NO )\n';
excute    = input(reconfirm);

if (excute == 1)
    %integrate input voltage scandata
    unit_ltp_ltd_input_data       = [[flip(mydc_chennal_adder(0, read_unit_pulse_input, 0), 2), tr_read_unit_pulse_input] ;...
                                     repmat(input_unit_set_mode, PARA.NUMB_OF_SET, 1) ;...
                                     repmat(input_unit_reset_mode, PARA.NUMB_OF_RESET, 1)];
...
% ----------------------------------
currentFolder = pwd;
    if ~isfolder('c_LTPLTD_Endu1T1R_minus_stanbyVer2')
        mkdir c_LTPLTD_Endu1T1R_minus_stanbyVer2
    end
    cd(currentFolder);
    cd('c_LTPLTD_Endu1T1R_minus_stanbyVer2');
...
fprintf("c_LTPLTD_Endu1T1R_minus_stanbyVer2 END\n");
%   Copyright (c) 2022 by ENTIS, All rights reserved.
% 
%   File name  : c_LTPLTD_Endu1T1R_minus_stanbyVer2.m
