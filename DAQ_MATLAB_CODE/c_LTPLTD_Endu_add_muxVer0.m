...
set_unit_pulse_input  = mydc_chennal_adder(PARA.MUX_ON, mydc_chennal_adder(0, set_unit_pulse_input, 0), PARA.MUX_SET_PULSE_ON);
read_unit_pulse_input = mydc_chennal_adder(PARA.MUX_ON, flip(mydc_chennal_adder(0, read_unit_pulse_input, 0), 2), PARA.MUX_READ_PULSE_ON);
reset_unit_pulse_input = mydc_chennal_adder(PARA.MUX_ON, mydc_chennal_adder(0, reset_unit_pulse_input, 0), PARA.MUX_RESET_PULSE_ON);
...
xlabel("Scan Data (#)")
ylabel("Voltage (V)")
legend({'GATE', 'DRAIN', 'MUX'},'Location','best')
grid on
...
if (excute == 1)
    ...
    unit_ltp_ltd_size    = size(unit_ltp_ltd_input_data, 1);
...
% ----------------------------------
    for cycle_number = 1:1:PARA.NUMB_OF_CYCLE
        ...
        if (cycle_number == 1)
            ...
            measured_data = readwrite(myDAQ, input_data);
            Variables     = -measured_data{unit_ltp_ltd_size*(PARA.NUMB_OF_LTPLTD_PER_CYCLE-1) + 1:end,:};
            Time          = measured_data.Time(1:unit_ltp_ltd_size);
        end
        % Time Check
...
% =========================================================================
        ...
        if (cycle_number == 1)
            for ltpltd_number = 1:1:PARA.NUMB_OF_LTPLTD_PER_CYCLE
                ...
                reset_mode_start_point = size(input_unit_set_mode, 1) * PARA.NUMB_OF_SET;
                [reset_endurance.current, reset_endurance.cycle] =...
                    myavg_readpulse(...
                    caculated_data.currnet,...
                    PARA.NUMB_OF_RESET,...
                    reset_mode_start_point + read_duration_end + unit_ltp_ltd_size * (ltpltd_number - 1),...
...
% ----------------------------------
currentFolder = pwd;
    if ~isfolder('c_LTPLTD_Endu_add_muxVer0')
        mkdir c_LTPLTD_Endu_add_muxVer0
    end
    cd(currentFolder);
    cd('c_LTPLTD_Endu_add_muxVer0');
...
fprintf("c_LTPLTD_Endu_add_muxVer0 END\n");
%   Copyright (c) 2022 by ENTIS, All rights reserved.
% 
%   File name  : c_LTPLTD_Endu_add_muxVer0.m
