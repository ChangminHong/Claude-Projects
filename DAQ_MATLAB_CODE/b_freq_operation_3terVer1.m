...
% [1] Parameter
%
% 0 input  (Dev1_ai0) = 'analog_output';
% 1 input  (Dev1_ai1) = 'voltage_output';
% 0 output (Dev1_ao0) = 'analog_input';
% =========================================================================
% --------------------------
% [1-1] Setting Param
%
...
% ----------------------------------
    % Caculate Output I, V
    measured_data.Dev1_ai0 = -measured_data.Dev1_ai0 * PARA.SENSITIVITY;
    % Drow Output I-V Grape
    subplot(2,1,2)
    yyaxis right
    plot(measured_data.Time, measured_data.Dev1_ai0, 'r-');
    title('Input & Output')
    xlabel("Time (sec)")
    ylabel("Current (A)")
    legend({'input', 'output current'},'Location','best')
    grid on
    ...
    yyaxis right
    plot(measured_data.Time, measured_data.Dev1_ai0, 'r-');
    title('Output Current & Voltage')
    xlabel("Time (sec)")
    ylabel("Current (A)")
    yyaxis left
    fig = plot(measured_data.Time, measured_data.Dev1_ai1, 'k-');
    ylabel("Voltage (V)")
    legend({'output voltage', 'output current'},'Location','best')
    grid on
...
currentFolder = pwd;
    ...
    if ~isfolder('b_freq_operation_3terVer1')
        mkdir b_freq_operation_3terVer1
    end
    cd(currentFolder);
    cd('b_freq_operation_3terVer1');
    ...
    writematrix(measured_data.Dev1_ai1, strcat(save_index, '_measured_data.voltage.txt'));
...
fprintf("b_freq_operation_3terVer1 END\n");
%   Copyright (c) 2022 by ENTIS, All rights reserved.
% 
%   File name  : b_freq_operation_3terVer1.m
